...
export async function getHistory(token, deposit) {
  const res = await fetch("http://bank.com/api/history", {
    headers: {
      token: token,
      name: deposit,
    },
...
export async function createTransaction(token, deposit, action, amount) {
  const res = await fetch("http://bank.com/api/deposit", {
    headers: {
      token: token,
      name: deposit,
